---
id: "incident-management"
title: "Quản lý sự cố"
description: "Tổng quan về dịch vụ Incident Management trên FPT Cloud"
sidebar_label: "Quản lý sự cố"
sidebar_position: 1
draft: true
---

# Quản lý sự cố

FPT Smart Cloud ra mắt tính năng **Incident Management** — công cụ quản lý sự cố thông minh giúp bạn nhanh chóng ứng phó với các sự kiện bất ngờ. Với khả năng tích hợp Jira, webhook và các công cụ thông báo, Incident Management đơn giản hóa quy trình xử lý sự cố, giảm thời gian và công sức cần thiết.

## Nội dung tài liệu

### Thiết lập ban đầu
- [Thiết lập ban đầu](./initial-setup.md)

### Cấu hình tích hợp (Integrations)
- [Tổng quan cấu hình integrations](./cau-hinh-integrations.md)
  - [Incoming webhook](./incoming-webhook.md)
  - [Grafana webhook](./grafana-webhook.md)
  - [Outgoing webhook](./outgoing-webhook.md)
  - [Atlassian Jira](./jira.md)
  - [Slack](./slack.md)
  - [Telegram](./telegram.md)
  - [Microsoft Teams](./microsoft-teams.md)

### Cấu hình settings
- [Tổng quan cấu hình settings](./cau-hinh-settings.md)
  - [Incident label](./incident-label.md)
  - [Incident severity](./incident-severity.md)
  - [Incident status](./incident-status.md)

### Tạo incident
- [Tổng quan tạo incident](./tao-incident.md)
  - [Khai báo incident trực tiếp](./declare-incident.md)
  - [Tạo incident qua incoming webhook](./create-by-incoming-webhook.md)

### Quản trị incident
- [Tổng quan quản trị incident](./quan-tri-incident.md)
  - [Tìm kiếm và lọc incidents](./filter-incidents.md)
  - [Xem chi tiết incident](./xem-chi-tiet-incident.md)
  - [Ghi lại hoạt động xử lý](./post-incident-activity.md)
  - [Thay đổi trạng thái incident](./thay-doi-trang-thai-incident.md)
  - [Thay đổi mức độ ảnh hưởng](./thay-doi-muc-do-anh-huong-incident.md)
  - [Xuất activity log dạng markdown](./copy-activities-as-markdown.md)

### Trực quan hóa dữ liệu
- [Trực quan dữ liệu incident](./truc-quan-du-lieu-incident.md)
