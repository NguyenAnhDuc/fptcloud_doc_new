...
description: "Configure Terraform with the OpenStack Provider to authenticate and manage FPT Cloud resources."
...
## Initial setup and service authentication
...
|---|---|---|
| `auth_url` | URL of the Identity Service | `OS_AUTH_URL` |
...
| `tenant_name` | VPC name provided by FPT Cloud | `OS_PROJECT_NAME` |
...
Other optional configurations can be found in the [Configuration Reference](https://registry.terraform.io/providers/terraform-provider-openstack/openstack/latest/docs#configuration-reference).
...
| `external_network_id` | ID of external gateway. A router with an external gateway is required if compute instances or load balancers will use floating IPs. | |
| ... | | |
...
| `shared` | Specifies whether the network resource can be accessed by any tenant. | bool | false | No |
| ... | | | | |
...
| Parameter | Description | Type | Default | Required |
|---|---|---|---|---|
| `network_id` | ID of the network from the previous step. | string | | Yes |
| `cidr` | CIDR representing the IP range for this subnet, based on IP version. | string | | Yes |
| `ip_version` | IP version of the subnet. | int | 4 | No |
| `enable_dhcp` | Enable DHCP for the subnet. | bool | true | No |
| `allocation_pools` | IP pools for DHCP. | list | | No |
| `gateway_ip` | Default gateway IP. | string | | No |
| `host_routes` | List of default static host routes. | list | | No |
| `dns_nameservers` | List of DNS servers for the subnet. | list | | No |
| `prefixlen` | Prefix length of the subnet. | int | | No |
| ... | | | | |
...
|---|---|---|---|---|
| `network_id` | ID of the network from the previous step. | string | | Yes |
| `mac_address` | Fixed MAC address for the port. | string | | No |
| `name` | Port name. | string | | No |
| `fixed_ips` | Fixed IP for the port. | list | | No |
| `allowed_address_pairs` | IP/MAC address pairs of additional IP addresses that can be active on this port. | list | | No |
| `security_group_ids` | A list of security group IDs to apply to the port. | list | | No |
| ... | | | | |

#### 5. [Floating IP](https://registry.terraform.io/providers/terraform-provider-openstack/openstack/latest/docs/resources/networking_floatingip_v2)
...
| Parameter | Description | Type | Default | Required |
|---|---|---|---|---|
| `pool` | The name of the pool from which to obtain the floating IP. E.g. `provider-net4`. Changing this creates a new floating IP. | string | | Yes |
...
| Parameter | Description | Type | Default | Required |
|---|---|---|---|---|
| `security_group_id` | The security group ID the rule should belong to. Must be an OpenStack ID of a security group in the same tenant. | string | | Yes |
...
| `remote_group_id` | The remote group ID. Must be an OpenStack ID of a security group in the same tenant. | string | | No |
...
This example deploys a web application on FPT Cloud using Terraform:

1. Set up Terraform.
2. Create a new project directory containing a `main.tf` file with the following content:
...
4. The Terraform CLI shows the result:

   [![Terraform CLI result](/img/migrated/image-1702440353764-3c9f7ced.png)](/img/migrated/image-1702440353764-3c9f7ced.png)

5. Access the floating IP to view the deployed webapp:

   [![Webapp result](/img/migrated/image-1702440375843-4b51f2bf.png)](/img/migrated/image-1702440375843-4b51f2bf.png)

6. Clean up all resources created by the IaC stack:
