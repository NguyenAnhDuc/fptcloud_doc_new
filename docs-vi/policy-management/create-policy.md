...
description: "Hướng dẫn khởi tạo Policy Management trên FPT Cloud với phân quyền Tenant owner."
...
| Assign tags to the new **instance** immediately after creation | Specific rule | Instance name include text | Hệ thống tự động gắn tag cho instance ngay khi tạo nếu instance thỏa mãn điều kiện tên có chứa ký tự đã config |
...
| Assign tags to the new **instance & related resources** immediately after creation (related resource = Storage, IP, Subnet, Secgroup) | Specific rule | Instance name include text | Hệ thống tự động gắn tag cho instance và các resource liên quan (Storage, IP, Subnet, Secgroup) ngay khi tạo nếu instance thỏa mãn điều kiện tên có chứa ký tự đã config |
...
| Assign tags to the new **storage** immediately after creation | Specific rule | Instance name include text | Hệ thống tự động gắn tag cho storage ngay khi tạo nếu storage thỏa mãn điều kiện tên có chứa ký tự đã config |
...
**Bước 3:** Chọn **Create policy**. Hệ thống sẽ tiến hành khởi tạo Policy mới và hiển thị ở trang **Policy Management**.

[![file](/img/migrated/image-1719821699202-a820ed2a.png)](/img/migrated/image-1719821699202-a820ed2a.png)

:::note
- Người dùng chỉ áp dụng được 1 policy duy nhất cho mỗi user group.
- Để các policy được chặt chẽ, khi khởi tạo các policy cần truy cập vào các port đặc biệt, FPT Cloud khuyến khích người dùng khởi tạo thêm các policy liên quan bao gồm:
