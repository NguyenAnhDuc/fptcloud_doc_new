---
id: "fpt-kafka"
title: "FPT Kafka"
description: "Tổng quan về dịch vụ FPT Kafka trên FPT Cloud."
sidebar_label: "FPT Kafka"
sidebar_position: 1
---

# FPT Kafka

FPT Kafka là dịch vụ nhắn tin phân tán giúp bạn gửi và nhận dữ liệu theo thời gian thực, phù hợp để xây dựng Message Queue, Data Streaming Pipeline và Data Replication. Dịch vụ cung cấp giao diện quản lý topic, consumer group, credential và ACL tích hợp sẵn.

## Nội dung tài liệu

- [Thiết lập ban đầu](./initial-setup.md)
- [Bắt đầu nhanh](./quick-starts.md)
- [Quản lý topic](./manage-topics.md)
  - [Tạo topic](./create-topics.md)
  - [Xem topic](./view-topics.md)
  - [Chỉnh sửa topic](./edit-topics.md)
  - [Xóa topic](./delete-topics.md)
- [Quản lý credential](./manage-credentials.md)
  - [Tạo credential](./create-credentials.md)
  - [Chỉnh sửa credential](./edit-credentials.md)
  - [Xóa credential](./delete-credentials.md)
- [Quản lý ACL](./manage-acls.md)
  - [Tạo ACL](./create-acls.md)
  - [Xóa ACL](./delete-acls.md)
- [Quản lý consumer group](./manage-consumer-groups.md)
  - [Tạo consumer group](./create-consumer-groups.md)
  - [Xem consumer group](./view-consumer-groups.md)
  - [Xóa consumer group](./delete-consumer-groups.md)
- [Demo](./demo.md)
