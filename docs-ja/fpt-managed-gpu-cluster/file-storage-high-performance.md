...
Chọn đúng network của server Metal Cloud làm worker node trong GPU cluster, các mountpoint của File Storage – High Performance sẽ được hiển thị phụ thuộc theo network này. 
  * ステップ2: Khi đã có MountPoint trong network metal cloud mong muốn, thực hiện enable File Storage – High Performance và chọn đúng MountPoint mong muốn.

[![](/img/migrated/4-1-8169c048.png)](/img/migrated/4-1-8169c048.png)
注意: Nếu tenant chưa được active dịch vụ File Storage – High Performance sẽ hiển thị thông báo sau, cần gửi yêu cầu active dịch vụ trước khi thực hiện tích hợp trên Managed GPU Cluster.
[![](/img/migrated/3-4-543f9c6b.png)](/img/migrated/3-4-543f9c6b.png)
  * ステップ4: Kiểm tra lại toàn bộ thông tin High Performance Storage Integration và tiến hành tạo cụm Managed GPU Cluster
...
➤ 1.2. Tích hợp với Managed GPU cluster tạo mới: 
  * ステップ1: Trên menu của FPT Portal chọn AI Infrastructure → Managed GPU Cluster → chọn cluster đã có để tích hợp File Storage – High Performance 

[![](/img/migrated/7-3-1d6e69ee.png)](/img/migrated/7-3-1d6e69ee.png)
注意: Managed GPU cluster tích hợp File Storage – High Performance phải ở trạng Succeeded (Running) mới có thể thực hiện tích hợp 
  * ステップ2: Trong tab Essential Properties → High Performance Storage Integration, click Enable High Performance Storage → chọn MountPoint trong list sau đó click button “Confirm”.
...
Chỉ hủy tích hợp File Storage – High Performance khi status của cluster là Succeeded (Running). Trước khi hủy bỏ tích hợp cần xóa toàn bộ PVC trong cluster sử dụng mountpoint đã chọn, việc hủy tích hợp không tự động xóa dữ liệu được ghi bởi Kubernetes trong thư mục của MountPoint. 
  * ステップ1: Trên menu của FPT Portal chọn AI Infrastructure → Managed GPU Cluster → chọn cluster đã tích hợp File Storage – High Performance 

[![](/img/migrated/7-3-1d6e69ee.png)](/img/migrated/7-3-1d6e69ee.png)
  * ステップ2: Trong tab Essential Properties → High Performance Storage Integration → disintegrate → Confirm 
...
注意: Không chỉnh sửa cấu hình storageclass mặc định của cluster nếu người dùng thay đổi cấu hình đó, nó sẽ tự động được rollback lại cấu hình ban đầu của hệ thống.
...
注意: Không thể giảm dung lượng (chỉ tăng). Nếu PVC đang được sử dụng bởi Pod, hệ thống sẽ tự động resize dung lượng của mountPath trong Pod (resize volume online)
