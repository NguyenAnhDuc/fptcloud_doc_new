...
**Type が sink、Database が MySQL の connector を作成します**
...
**ステップ 3:** **CDC service** の詳細画面 > **Connectors** タブを選択 > **Create a connector** をクリック ![image-1](images/mysql-sink-connector/img-001.png)
...
注意: connector 名には半角英小文字 a-z または数字 0-9 を使用できます。スペースは使用できません。スペースの代わりに「-」を使用してください。
  ...
  * **Database (必須):** **MySQL** を選択 ![image-2](images/mysql-sink-connector/img-002.png)
...
Properties 画面の情報を入力します
  ...
  * **Manual configuration** を選択した場合 — 以下を入力:
    ...
    * **Port** (必須): MySQL サーバーポート、デフォルト: `3306`
    ...
    * **Topics** (必須): Connector が consume してターゲット database にデータを sink する topic のリスト。「,」で区切ります ![image-3](images/mysql-sink-connector/img-003.png)
  ...
  * **From Database Engine** を選択した場合 — 以下を入力:
    ...
    * **Topics** (必須): Connector が consume してターゲット database にデータを sink する topic のリスト。「,」で区切ります ![image-4](images/mysql-sink-connector/img-004.png)
...
以下の情報を入力します:
  ...
  * **Mode** (必須): メッセージを処理できない場合の Connector の動作
    ...
    * **All**: エラーメッセージは指定した topic に送信されます ![image-5](images/mysql-sink-connector/img-005.png)

**ステップ 7**: **Next** をクリックして **Review** 画面に進みます ![image-6](images/mysql-sink-connector/img-006.png)
