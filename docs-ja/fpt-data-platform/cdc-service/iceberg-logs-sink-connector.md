...
**Type が sink、Database が Iceberg (logs) の connector を作成します**
...
**ステップ 3:** **CDC service** の詳細画面 > **Connectors** タブを選択 > **Create a connector** をクリック ![image-1](images/iceberg-logs-sink-connector/img-001.png)
...
注意: connector 名には半角英小文字 a-z または数字 0-9 を使用できます。スペースは使用できません。スペースの代わりに「-」を使用してください。
  ...
  * **Database** (必須): **Iceberg (logs)** を選択 ![image-2](images/iceberg-logs-sink-connector/img-002.png)
...
以下の情報を入力します:
  ...
  * **Topics** (必須): source connector から送信されるデータの topic を選択 ![image-3](images/iceberg-logs-sink-connector/img-003.png)
...
以下の情報を入力します:
  ...
  * **Mode**: メッセージを処理できない場合の Connector の動作
    ...
    * **None**: エラーが発生した場合、connector は処理を停止します ![image-4](images/iceberg-logs-sink-connector/img-004.png)

**ステップ 7:** **Next** をクリックして **Review** 画面に進みます ![image-5](images/iceberg-logs-sink-connector/img-005.png)
