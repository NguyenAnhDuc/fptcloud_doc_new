...
**Requirements:**
  * Quota CPU, GPU, RAM, Storage, Instance: đáp ứng đủ for cấu hình Worker Group mới mong muốn. Nếu muốn sử dụng Autoscale, số lượng GPU phải đáp ứng đủ số Max node tăng thêm mong muốn.
  * 01 Network subnet: Network sử dụng for Kubernetes Nodes, subnet cần có Static IP Pool.

Các bước thực hiện as follows: 
**Step 1** : Truy cập portal FPT Cloud console.fptcloud.com, chọn mục Kubernetes, bấm ando cluster cần thay đổi, mục Node Pools, bấm ando “Edit Workers”. 
...
Ngoài the thông tin khởi tạo for Kubernetes thông thường, cần chọn cấu hình for GPU in Worker Group:
