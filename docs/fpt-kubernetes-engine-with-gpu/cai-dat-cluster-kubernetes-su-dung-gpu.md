...
✅ **Requirements bắt buộc:**
  * Quota CPU, GPU, RAM, Storage, Instance: đáp ứng đủ for cấu hình cluster Kubernetes mong muốn. Nếu muốn sử dụng Autoscale, số lượng GPU phải đáp ứng đủ số Max node mong muốn (chú ý phần cài đặt Min node and Max node). 
...
**Step 2:** Enter thông tin cơ bản of cụm cluster sau that click button 
...
➤ **Public** : Nếu khách hàng cần truy cập Kubernetes API Endpoint from bên ngoài VPC, sử dụng tùy chọn this. 
➤ **Public & Private**: Dùng when cần Endpoint can truy cập công khai and muốn whitelist IP truy cập ando apiserver endpoint this.
➤ **Private** : Dùng when chỉ cần truy cập ando Endpoint in nội bộ VPC. 
[![](/img/migrated/Screenshot_3-3db7fd69.png)](/img/migrated/Screenshot_3-3db7fd69.png)
⚠️ **Note:**
  * Khách hàng cần chọn Cluster Endpoint Access phù hợp dựa trên yêu cầu security and kiến trúc network of hệ thống. 
  * Nếu chọn Public & Private or Private, will appears thêm trường Allow CIDR to nhập danh sách the dải địa chỉ IP có permission truy cập ando Endpoint of Kubernetes Cluster. 
...
⚠️ Note: Nếu cần security cao, khách hàng should hạn chế CIDR chỉ for phép the dải IP nội bộ thay vì 0.0.0.0/0. 
**Step 3** : Enter ando thông tin cụm Kubernetes cần khởi tạo. Ngoài the thông tin khởi tạo for Kubernetes thông thường, cần chọn cấu hình for GPU in Worker Group: 
...
_Chú ý:_
  ...
  3. Đối with lựa chọn “Pre-install”, cụm of khách hàng will is thêm Nvidia GPU driver a theh tự động. 
  4. Đối with lựa chọn “User-install”, khách hàng can cài đặt thủ công GPU driver to lựa chọn version driver phù hợp. 

**Step 4** : Ấn Create and kiểm tra lại the thông tin khởi tạo. 
...
Đối with card H100, portal không hỗ trợ tạo worker GPU là worker group base, khách hàng vui lòng tạo worker GPU from worker group 2 trở đi.
  * **Worker Group base:**
...
**Step 3:** Configure Nodes Pool theo nhu cầu sử dụng sau that click button 
Đối with card H200, portal không hỗ trợ tạo worker GPU là worker group base, khách hàng vui lòng tạo worker GPU from worker group 2 trở đi.
  * **Worker Group base:**
