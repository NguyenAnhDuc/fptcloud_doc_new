...
**Requirements:**
  ...
  * Số lượng GPU must đáp ứng đủ số Min node + 1 to the Worker Node can rollout config cấu hình. Nếu sử dụng Autoscale, số lượng GPU must đáp ứng đủ số Max node mong muốn. 
  * 01 Network subnet: Network sử dụng for Kubernetes Nodes, subnet need to có Static IP Pool. 

Các bước thực hiện as follows: 
**Step 1** : Truy cập portal FPT Cloud [console.fptcloud.com](https://console.fptcloud.com/), chọn mục Kubernetes, bấm ando cluster need to thay đổi, mục Node Pools, bấm ando biểu tượng "Edit Workers”. 
[![](/img/migrated/Picture8-2-eb37b7ec.png)](/img/migrated/Picture8-2-eb37b7ec.png)
**Step 2** : Ngoài the information cấu hình for Worker Group thông thường, need to chọn cấu hình for GPU: 
...
_⚠️ Chú ý:_
  * Thay đổi phương thức GPU sharing will whenến toàn bộ the workload liên quan to GPU need to is deploy lại should before thay đổi user need to scale ứng dụng về 0 to tránh gặp lỗi.
