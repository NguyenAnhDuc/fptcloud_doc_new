...
**Step 1** : Chọn Card NIC cần thay đổi địa chỉ IP. 
**Save ý** : Hành động this can ảnh hưởng to the service như Kubernetes, Load Balancer,...Bạn cần cân nhắc before sử dụng. Sau when thay đổi, to Instance hoạt động bình thường, the you cần thay đổi thông tin về Network in file config of hệ điều hành. Link hướng dẫn nhanh tại: [**How to change Config file**](../change-network-config/index.md).
...
Nhằm tối ưu trải nghiệm for khách hàng, Unify Portal hỗ trợ gợi ý 1 địa chỉ IP thuộc Subnet Mask of khách hàng.
