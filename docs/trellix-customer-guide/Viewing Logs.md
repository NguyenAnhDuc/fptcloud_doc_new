---
id: "Viewing Logs"
title: "Viewing Logs"
description: "From the menu, select Threat Event Log."
sidebar_label: "Viewing Logs"
sidebar_position: 6
pagination_next: null
---

# Viewing Logs

From the menu, select Threat Event Log.
[![file](/img/migrated/image-1765765683274-68178a40.png)](/img/migrated/image-1765765683274-68178a40.png)
This section displays security events for all servers with installed agents, including malware infections, exploits, etc.
[![file](/img/migrated/image-1765765697231-084be1af.png)](/img/migrated/image-1765765697231-084be1af.png)
Alternatively, for a specific server: Go to System Tree > Group > System, then double-click the server.
[![file](/img/migrated/image-1765765708664-117612a4.png)](/img/migrated/image-1765765708664-117612a4.png)
Select the Threat Events tab to view events for that server.
[![file](/img/migrated/image-1765765721659-77184455.png)](/img/migrated/image-1765765721659-77184455.png)
On each server, users can also right-click the Trellix icon and select Trellix ENS.
[![file](/img/migrated/image-1765765732756-8f460d01.png)](/img/migrated/image-1765765732756-8f460d01.png)
In the Event Log section, similar Threat Events will be displayed.
[![file](/img/migrated/image-1765765789911-5df77202.png)](/img/migrated/image-1765765789911-5df77202.png)
View Log Folder: The directory containing Endpoint Security log files, including scheduled scanning tasks and related activities.
[![file](/img/migrated/image-1765765814382-c01507d6.png)](/img/migrated/image-1765765814382-c01507d6.png)
