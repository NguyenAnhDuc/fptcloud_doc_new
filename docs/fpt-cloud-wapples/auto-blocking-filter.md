...
Thực hiện đưa ando ‘Blocking list’ the ip thỏa mãn theo 'HTTP Dos/Risk Level by Rule' has been thiết lập
[![file](/img/migrated/image-1714116699139-ec997656.png)](/img/migrated/image-1714116699139-ec997656.png)
Risk Level by Rule: thiết lập the trường điều kiện to đưa ip ando blocklist includes:
  * Duration: Phạm vi thời gian tích lũy risk level
  * Cumulative Risk Level: Mức độ rủi ro tích lũy corresponding with mỗi rule risk in Policy Settings. Khi đạt to ngưỡng, tự động block ip and đưa ando ‘Blocking list’
  * Blocking Duration: Thời gian thực hiện block. Tự động xóa bỏ khỏi IP Filter and danh sách block when hết thời gian HTTP DoS: Set up the trường điều kiện nhằm chống DoS and đưa ip attack ando blacklist includes:
  ...
  2. Slow HTTP POST DoS (feature thêm)
     ...
     * Exclude from Auto Blocking: Set up ngoại lệ
