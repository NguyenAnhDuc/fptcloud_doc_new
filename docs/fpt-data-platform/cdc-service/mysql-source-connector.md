...
**Create a connector with Type: source, Database: MySQL**

**Pre-condition: CDC service status is healthy**

The MySQL source connector uses the binary log of MySQL to perform CDC. However, MySQL is configured to purge binlogs after a period of time. Therefore, when the MySQL connector is initialized, it will perform an _initial consistent snapshot_ before starting to read from the binlogs to ensure data consistency.
...
**Step 3:** On the **CDC service** detail screen > Select the **Connectors** tab > click **Create a connector** ![image-1](images/mysql-source-connector/img-001.png)
...
Note: The connector name may contain lowercase letters a-z or digits 0-9. Spaces are not allowed; use "-" as a separator instead.
  ...
  * **Database** (required): select MySQL ![image-2](images/mysql-source-connector/img-002.png)
...
Note: The Database server ID must be a number greater than 1000 and less than 9999.
    ...
    * **Topic prefix** (required): List of topics the Connector will consume and sink data into the target database, separated by "," ![image-3](images/mysql-source-connector/img-003.png)
...
Note: The Database server ID must be a number greater than 1000 and less than 9999.
    ...
    * **Topics** (required): List of topics the Connector will consume and sink data into the target database, separated by "," ![image-4](images/mysql-source-connector/img-004.png)
...
Select one of the following modes:
  ...
  * **Never**: The Connector will not snapshot existing data in the tables but will only listen to data change events on the tables
    ...
    * **Column** (optional): name of a data column to retrieve from the table ![image-5](images/mysql-source-connector/img-005.png)

**Step 7:** Click **Next** to proceed to the **Review** screen ![image-6](images/mysql-source-connector/img-006.png)
