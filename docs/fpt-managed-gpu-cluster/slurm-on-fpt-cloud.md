...
➤ **FPT Managed GPU Cluster** dựa trên nền tảng mã nguồn mở K8s giúp tự động hoá triển khai, nhân rộng and quản lý the ứng dụng container. FPT Managed GPU Cluster tích hợp đầy đủ the thành phần: Container Orchestration, Storage, Networking, Security, PaaS cung cấp for customer môi trường tốt nhất to phát triển and triển khai ứng dụng trên Cloud.
➤ **FPT Managed GPU Cluster** là a mô hình cung cấp service Managed GPU Cluster of FKE. Với MANAGED GPU CLUSTER, FPT Cloud quản trị toàn bộ thành phần control-planes, người sử dụng will triển khai and quản trị the Worker Nodes. MANAGED GPU CLUSTER giúp người sử dụng tập trung ando việc triển khai ứng dụng mà không need to tốn nguồn lực ando việc quản trị K8s Cluster.
...
**Những điều need to lưu ý before sử dụng Managed GPU Cluster?**
**· Vị trí đặt Managed GPU Cluster:** Vị trí địa lý (Region) can will ảnh hưởng đến tốc độ truy cập đến server in quá trình sử dụng. Bạn nên chọn Region gần nhất with đối tượng phát sinh traffic to tối ưu is tốc độ. 
**· Số lượng the Nodes and cấu hình of fromng Nodes need to sử dụng :** Các account FPT Cloud đều is cấp a hạn mức nhất định for the tài nguyên Ram, CPU, Storage, IP… Vì vậy, customer nên xác định số lượng tài nguyên need to sử dụng and giới hạn tối đa need to đáp ứng to FPT Cloud hỗ trợ you tốt nhất.
**1. Giới thiệu Slurm and Slurm on Kubernetes**
**1.1 Giới thiệu Slurm**
Slurm là a nền tảng mã nguồn mở mạnh mẽ dùng to quản lý tài nguyên cụm and lập lịch công việc (job). Nó is thiết kế nhằm tối ưu hiệu năng and hiệu quả for the siêu máy tính and cụm tính toán lớn. Các thành phần chính of nó hoạt động cùng nhau to đảm bảo system đạt hiệu suất cao and tính linh hoạt. Hình sau đây minh họa theh thức hoạt động of Slurm.
[![](/img/migrated/Picture1-1-00c51d67.png)](/img/migrated/Picture1-1-00c51d67.png)
**slurmctld** : là trình nền điều whenển of Slurm. Được xem như “bộ não” of Slurm, slurmctld monitoring tài nguyên system, lập lịch job and quản lý status cụm. Để tăng tính tin cậy, you can cấu hình a slurmctld thứ cấp nhằm tránh gián đoạn service if slurmctld chính gặp sự cố, đảm bảo system luôn sẵn sàng cao.
**slurmd** : là trình nền nút of Slurm. slurmd is triển khai trên mỗi nút tính toán. Nó nhận lệnh from slurmctld and quản lý job, includes khởi chạy and thực thi job, report status job and chuẩn bị for the lệnh job mới. slurmd thatng vai trò interface to giao tiếp trực tiếp with tài nguyên tính toán and là cơ sở to lập lịch job.
**slurmdbd** : là trình nền database of Slurm. slurmdbd là thành phần tùy chọn nhưng rất quan trọng for quản lý lâu dài and kiểm toán in the cụm lớn, vì nó duy trì a database tập trung to save lịch sử job and information kế toán. slurmdbd can tổng hợp dữ liệu from nhiều cụm do Slurm quản lý, giúp đơn giản hóa and nâng cao hiệu quả quản lý dữ liệu.
**SlurmCLI:** cung cấp the lệnh sau to hỗ trợ quản lý job and monitoring system:
...
**1.2 Why Slurm K8s?**
Cả Slurm and Kubernetes đều can thatng vai trò là system quản lý tải công việc for huấn luyện mô hình phân tán and tính toán hiệu năng cao (HPC) nói chung.
Mỗi system đều có điểm mạnh and điểm yếu riêng, and việc đánh đổi giữa chúng là đáng kể. Slurm cung cấp khả năng lập lịch tiên tiến, hiệu quả, kiểm soát phần cứng details and khả năng kế toán, nhưng lại thiếu tính phổ quát. Ngược lại, Kubernetes can is sử dụng for nhiều mục đích khác ngoài huấn luyện (ví dụ: inferencing) and cung cấp khả năng tự động mở rộng and tự phục hồi tốt.
Thật đáng tiếc là hiện chưa có theh nào to kết hợp lợi ích of cả hai giải pháp this. Và vì nhiều công ty công nghệ lớn sử dụng Kubernetes như a lớp hạ tầng mặc định mà không hỗ trợ system huấn luyện mô hình chuyên dụng, nên a số kỹ sư ML thậm chí không có sự lựa chọn.
Việc sử dụng Slurm trên k8s for phép chúng tôi tái sử dụng khả năng tự động mở rộng and tự phục hồi of Kubernetes in Slurm, đồng thời triển khai a số feature độc đáo, mà vẫn giữ nguyên theh tương tác quen thuộc with system.
**1.3 Giới thiệu Slurm on FPT Cloud Managed GPU/K8s cluster**
Slurm Operator sử dụng tài nguyên tùy chỉnh SlurmCluster (CR) to định nghĩa the tập tin cấu hình need to thiết for việc quản lý the cụm Slurm and giải quyết the vấn đề liên quan đến quản lý tầng điều whenển. Điều this giúp đơn giản hóa việc triển khai and bảo trì the cụm is quản lý bởi Slurm. Hình dưới đây mô tả kiến trúc of Slurm trên FPTCloud Managed GPU/k8s cluster. Quản trị viên cụm can triển khai and quản lý a cụm Slurm thông qua SlurmCluster. Slurm Operator will tạo the thành phần điều whenển of Slurm in cụm dựa trên SlurmCluster. Một tập tin cấu hình Slurm can is gắn ando thành phần điều whenển thông qua a volume chia sẻ or a ConfigMap.
[![](/img/migrated/Picture2-1-d68031fc.png)](/img/migrated/Picture2-1-d68031fc.png)
Trong mô hình triển khai Slurm on K8s, the thành phần of a cụm slurm như login node, worker node, ... is biểu diễn thành the pods trên K8s, đồng thời in mô hình this, khái niệm shared-root volume is thực hiện, hiểu đơn giản thì chúng ta will triển khai a shared filesystem, file system tương đương with file system of a OS, mọi job after is chuyển đến worker node will đều is triển khai trên môi trường shared root volume this. Điều this đảm bảo rằng mọi worker node will luôn có cấu hình, the packages & status giống nhau mà không need to quản lý thủ công. Nói theh khác, when you cài đặt packages trên a node, the packages that will tự động appears trên the node còn lại.
Tất cả những gì you need to làm là định nghĩa về cụm slurm mong muốn of you tại Slurm cluster custom resource, Slurm operator will thực hiện việc triển khai & quản lý cụm Slurm thay for you về status you has been định nghĩa ở Slurm cluster CR.
**2. Hướng dẫn cài đặt**
...
**Step 1:** Cài đặt Slurm Operator, GPU Operator, Network Operator tại phần cài đặt GPU software & chờ for đến when status the phần this ở ready [![](/img/migrated/Picture3-2-863f4c91.png)](/img/migrated/Picture3-2-863f4c91.png)
...
**Trong that:**
**· jail-pvc** : mount ando worker node and login node, thatng vai trò như a shared sandbox, là môi trường the job is thực thi, cũng là môi trường user thao tác, dung lượng volume this ít nhất là 40Gi to chứa file system of a OS.
**· controller-spool-pvc** : Save information cấu hình về cụm, đặt tại controller node.
jail-pvc.yaml
...
controller-spool-pvc.yaml (không need to tạo with slurm operator version >= 2.0.0)
...
**Note:**
➤ Các volume this đều là the volume bắt buộc must có, chúng đều need to cung cấp cơ chế Read write many and tên of the PVC this must giữ nguyên như trên.
...
Tại file values.yaml of folder vừa tải, you need to chỉnh a số trường quan trọng như:  
| Tên trường  | Mô tả  |  
| --- | --- |  
| slurmNodes.worker.size  | số lượng worker nodes  |  
| slurmNodes.worker.size.spool.volumeClaimTemplateSpec.storageClassName  | storage class of persistent volume dùng to save status worker nodes  |  
| slurmNode.login.sshRootPublicKeys  | list the public key of user root in cụm Slurm  |  
| SlurmNode.accounting.mariadbOperator.storage.volumeClaimTemplate.storageClassNam  | storage class of persistent volume dùng to save dữ liệu tại slurmdbd node  |  
...
**Step 4:** Chờ for đến when toàn bộ pod Slurm ở status running Quá trình this mất khoảng 20 phút ở lần đầu tiên cài slurm cluster trên cụm k8s, includes 2 phase, phase 1 chạy job setup and phase 2 cài đặt the pod slurm components
...
Sau when login successfully ando cụm Slurm, you can kiểm tra hoạt động of cụm slurm bằng việc train model mingpt theo guide sau:
...
**Step 2:** chuyển hướng đến folder minGPT-ddp & cài đặt the gói need to thiết
...
Nhờ cơ chế shared root, chúng ta chỉ need to chạy lệnh trên a lần, the package this will tự sync trên toàn bộ the node worker khác.
Note: in môi trường production, chúng tôi khuyến nghị việc dùng conda evironment/ container to create môi trường train thay vì cài trực tiếp the package ở môi trường global như trên.
...
Tại phần 2 of guide chạy Slurm trên K8s, chúng tôi has been guide you chỉnh the tham số quan trọng nhất. Tại phần this, chúng ta will đi ando tìm hiểu thêm về the tham số/thuộc tính is định nghĩa for a Slurm cluster, you cũng can đọc phần comment tại file values.yaml of slurm cluster custom resource has been tải ở phần 2 to biết thêm information về the trường.  
| Thuộc tính  | giá trị mẫu  | Mô tả  |  
| --- | --- | --- |  
| clusterName  | "fpt-hpc"  | tên cụm (lưu ý không đổi)  |  
| k8sNodeFilters  | N/A  | Chia cụm k8s ra làm hai list: the node GPU (deploy slurm worker) and the node no-gpu to deploy the thành phần khác, in trường hợp cụm chỉ có node GPU, hai list node this can giống nhau.  |  
...
requests:  
storage: 10Gi  
storageClassName: xplat-nfs  | cấu hình accounting node, tại đây chúng tôi sử dụng mariadb operator to create database, you cũng can sử dụng database ngoài (đọc thêm tại phần values.yaml)  |  
...
**4.1 Add user/ login**
Add user
Để thêm ssh key for root, you đơn giản chỉ need to edit slurm cluster CR:

```
Copykubectl edit SlurmCluster  fpt-hpc -n fpt-hpc
```

Tại phần cấu hình login node, chuyển hướng đến thuộc tính sshRootPublicKeys, thêm public key mong muốn of you.
...
Để chỉnh sửa số lượng worker nodes nói riêng and số lượng the nodes thuộc loại khác nói chung, chúng ta đơn giản chỉ need to edit lại slurm cluster CR:
...
Note: 
-Khi scale up số lượng worker nodes, node mới will is tự động thêm ando list worker nodes tại slurm controller node and sẵn sàng to chạy job.
-Khi scale down nodes, you need to xóa node thủ công tại slurm controller bằng lệnh:

```
Copyscontrol delete nodeName=node_name_to_delete
```

-Danh sách nodes in a cluster luôn là worker-[0, (size - 1)]
**4.3 Migrate Slurm cluster sang k8s cluster khác**
Nhờ sự linh hoạt of k8s and network file storage, chúng ta can dễ dàng chuyển slurm cluster from a cụm k8s this sang cụm k8s khác, việc must làm là mounting & tạo lại jail-pvc trên cụm slurm k8s mới and thực hiện lại the bước tạo slurm k8s.
**4.4 Mounting external volumes ando cụm slurm**
Để mount a volume ando slurm cluster, you need to tạo volume that trước, sau that triển khai volume that dưới dạng PV & PVC tại K8s. Ví dụ sau sử dụng dynamic provisioning to create PV/PVC this (in môi trường production chúng tôi khuyến nghị sử dụng static provisioning volume to đảm bảo an toàn dữ liệu)
**Step 1:** tạo PVC
...
Note: mount path need to giống nhau trên the worker node and login node
**4.5 Sử dụng docker**
...
Từ version 2.0.0, slurm operator hỗ trợ việc ssh trên the worker node, thay vì việc must dùng Kubernetes to shell ando worker nodes. User of the worker nodes đồng nhất with login nodes.
Với login nodes, mặc định when ssh ando IP of loadbalancer, loadbalancer will route ssh session đến a login node ngẫu nhiên.
