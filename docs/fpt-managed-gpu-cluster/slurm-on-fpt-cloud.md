...
**Step 1** : Cài đặt Slurm Operator, GPU Operator, Network Operator tại phần cài đặt GPU software & chờ cho đến khi trạng thái các phần này ở ready [![](/img/migrated/Picture3-2-863f4c91.png)](/img/migrated/Picture3-2-863f4c91.png)
**Step 2:** Tại cụm k8s, tạo sẵn các Persistent volume để chứa shared root space và dữ liệu của controller node.
...
Note: chỉnh version của slurm-cluster đúng với version của Slurm operator
...
**Step 4** : Chờ cho đến khi toàn bộ pod Slurm ở trạng thái running Quá trình này mất khoảng 20 phút ở lần đầu tiên cài slurm cluster trên cụm k8s, bao gồm 2 phase, phase 1 chạy job setup và phase 2 cài đặt các pod slurm components
...
**Step 2** : chuyển hướng đến folder minGPT-ddp & cài đặt các gói cần thiết
...
Note: trong môi trường production, chúng tôi khuyến nghị việc dùng conda evironment/ container để tạo môi trường train thay vì cài trực tiếp các package ở môi trường global như trên.
...
**Step 2** : khai báo volume này tại slurm cluster
...
**Step 3** : mount các volume trên vào các node login và worker trong cụm slurm
Tại login node:
...
Note: mount path cần giống nhau trên các worker node và login node
**4.5 Sử dụng docker**
