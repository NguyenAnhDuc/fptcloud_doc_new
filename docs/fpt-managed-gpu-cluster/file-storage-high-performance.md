...
**Save ý điều kiện to sử dụng High Performance Storage includes** : 
  1. Các điều kiện bắt buộc to khởi tạo Managed GPU cluster (Active service, quota metal cloud, ssh key, internal subnet LB, ...). Tham khảo in tài liệu khởi tạo Managed GPU Cluster [here](../fpt-managed-gpu-cluster/index.md).
  2. Đảm bảo service File Storage – High Performance has been is enable and cấp quota in tenant.
  3. Cần có MountPoint nào thuộc network of Metal cloud muốn sử dụng, di chuyển to tab File Storage – High Performance to tạo MountPoint mới theo hướng dẫn [here](../file-storage-high-performance/index.md)
**Enable service File Storage – High Performance with Managed GPU cluster trên giao diện FPTcloud Unify Portal.**
...
Note: Nếu tenant chưa is active service File Storage – High Performance will displayed notification sau, cần gửi yêu cầu active service before thực hiện tích hợp trên Managed GPU Cluster.
[![](/img/migrated/3-4-543f9c6b.png)](/img/migrated/3-4-543f9c6b.png)
  * Step 4: Check lại toàn bộ thông tin High Performance Storage Integration and tiến hành tạo cụm Managed GPU Cluster
...
Note: Managed GPU cluster tích hợp File Storage – High Performance phải ở trạng Succeeded (Running) mới can thực hiện tích hợp 
...
Chỉ hủy tích hợp File Storage – High Performance when status of cluster là Succeeded (Running). Trước when hủy bỏ tích hợp cần xóa toàn bộ PVC in cluster sử dụng mountpoint has been chọn, việc hủy tích hợp không tự động xóa dữ liệu is ghi bởi Kubernetes in thư mục of MountPoint. 
...
Trong a thời điểm chỉ can sử dụng a MountPoint trên Managed GPU cluster, to thay đổi MountPoint sử dụng in cluster, cần thực hiện lần lượt thao tác hủy tích hợp MountPoint cũ (mục 2.3) sau that tích hợp MountPoint mới for cluster (mục 2.2) 
...
➤ Create PersistentVolumeClaim (PVC) sử dụng storageclass sẵn có of hệ thống tạo ra for MountPoint has been tích hợp, do VOLUMEBINDINGMODE of storageclass là WaitForFirstConsumer should cần có Pod sử dụng PVC this to CSI thực hiện việc tạo PV and bind with PVC. 
Note: Không chỉnh sửa cấu hình storageclass mặc định of cluster if user thay đổi cấu hình that, nó will tự động is rollback lại cấu hình ban đầu of hệ thống.
...
➤ Để resize dung lượng of PVC cần thực hiện chỉnh sửa trực tiếp resource PVC tại trường spec.resources.requests.storage
Note: Không thể giảm dung lượng (chỉ tăng). Nếu PVC is is sử dụng bởi Pod, hệ thống will tự động resize dung lượng of mountPath in Pod (resize volume online)
