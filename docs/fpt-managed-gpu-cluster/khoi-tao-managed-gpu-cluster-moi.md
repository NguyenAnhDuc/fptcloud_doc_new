...
Quota Metal Cloud (Bare Metal HPC) must đáp ứng đủ số lượng mong muốn of Cluster. 
...
**Step 2:** Enter the information tại tab General Information of cụm Cluster sau that click button 
[![](/img/migrated/4-15b5adf9.png)](/img/migrated/4-15b5adf9.png)
  * General Information: 

**• Name** : Enter tên Cluster, tên of the Cluster must khác nhau and đúng quy tắc.
**• Network** : Chọn from dải subnet has been tạo for Bare Metal GPU Servers 
**• Version** : Chọn version Kubernetes phù hợp with ứng dụng hiện tại of customer. 
...
**Step 3:** Enter the information tại tab Nodes Pool of cụm Cluster sau that click button 
Những điểm need to lưu ý when tạo cụm Cluster MANAGED GPU CLUSTER: 
  * **Managed GPU Cluster** quản lý the Worker nodes thông qua Worker Group, là a nhóm includes the Worker nodes có cấu hình giống nhau. Người sử dụng can phân chia Worker Groups for the ứng dụng phù hợp. Hệ thống yêu cầu tối thiểu must có 01 Worker Group (Base), người sử dụng cannot xoá Worker Group this. 
...
**• Runtime** : Chọn container runtime, hiện tại, system mới chỉ hỗ trợ container runtime Containerd. 
...
**Save ý** : Người dùng when cấu hình label/taint for worker group trên Unify Portal will cannot xóa label/taint for node in worker group that bằng kubectl (Hệ thống will tự động thêm lại label/taint for node theo cấu hình trên Unify Portal) do that need to xóa cấu hình label/taint trên Unify Portal. 
➤ **Worker Group n** :
...
**Save ý** : Người dùng when cấu hình label/taint for Worker Group trên Portal will cannot xóa label/taint for node in Worker Group that bằng kubectl (Hệ thống will tự động thêm lại label/taint for node theo cấu hình trên Portal) do that need to xóa cấu hình label/taint trên Portal. 
...
Step 5: Màn hình Review & Create sẽ displayed the information cụm Cluster user has been cấu hình trước that and system tự động kiểm tra quota Bare Metal GPU server có đủ to khởi tạo Cluster hay không 
[![](/img/migrated/7-1-0c3879d8.png)](/img/migrated/7-1-0c3879d8.png)
Sau when system kiểm tra tài nguyên successfully, nhấn button Create a Managed GPU Cluster to proceed with tạo cụm Cluster.
