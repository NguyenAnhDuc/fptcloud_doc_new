...
**Giới thiệu feature vGPU in FPT Kubernetes engine**
...
**Hướng dẫn cài đặt**
**Step 1:** Cài đặt vGPU scheduler tại phần cài đặt GPU software & chờ for đến when status ready. [![](/img/migrated/Picture1.1-a0959517.png)](/img/migrated/Picture1.1-a0959517.png)
...
Note: 
Nếu you enable vGPU scheduler trên a worker group, tất cả the sharing mode khác như MIG, MPS, Time slicing need to is disable trên worker group that.
...
Có tối đa 48 container is phép chia sẻ cùng a GPU, tuy nhiên you chỉ nên sử dụng <= 20 vGPU trên fromng GPU to đảm bảo hiệu năng tổng thể.
...
Note: 
nvidia.com/vgpu: 1 có nghĩa là you muốn sử dụng vGPU sharing trên chỉ a GPU vật lý, if you request nvidia.com/vgpu: 2, you need to có 2 GPU vật lý.
...
**1. Hỗ trợ chia sẻ tài nguyên GPU a theh linh hoạt, the thông số can cài đặt including:**
...
Chú ý:
Việc cài đặt số lượng nvidia.com/gpu bằng 2 in a container có nghĩa là container of you sử dụng 2 vGPU đặt trên 2 GPU vật lý khác nhau, không must 2 vGPU trên cùng 1 GPU vật lý.
...
Không nên sử dụng 1 pod nhiều container and the container đều sử dụng GPU when you kích hoạt device plugin this.
