...
Sản phẩm Managed GPU Cluster is phát triển from Kubernetes Native and tích hợp thêm thêm the thành phần cloud provider ando Kubernetes in that có thành phần FPT Cloud Controller Manager. Thành phần this nhằm mục đích quản lý the worker nodes in cluster and the service dạng Load Balancer. Người dùng can có nhiều theh to expose a ứng dụng of mình ra bên ngoài internet to customer of user that can truy cập ando ứng dụng, service. Các theh that can includes việc tạo ingress for service, or tạo service dạng node port and gắn floating for worker node, or can sử dụng service dạng Load Balancer. 
...
| service.beta.kubernetes.io/fpt-load-balancer-type  | LBv1 including:  
...
Note: IP public need to is allocate ando VPC and ở status Inactive. Người dùng ando mục **Networking - > Floating Ips** to kiểm tra. 
