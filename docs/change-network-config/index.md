...
Change Network Config
##  Introduction 
Có nhiều lý do whenến you can cần thay đổi and đặt lại địa chỉ IP tĩnh for thiết bị of mình, chẳng hạn như cấu hình 1 server cố định or cần đặt quy luật for a nhóm máy tính in nội bộ. Một kịch bản nữa là when you thay đổi cấu hình card NIC trên hệ thống Cloud is cung cấp bởi FPT Smart Cloud, địa chỉ IP bên in máy will không tự cập nhật theo and you cần cấu hình thủ công (ngoài ra có a số phương pháp khác xử lý in tình huống this nhưng không is đề cập in nội dung bài viết).
**Những điều cần lưu ý before you thay** **đổi thông tin file config network** : 
Đặt địa chỉ IP tĩnh can giúp tránh xung đột network nhưng can whenến a số thiết bị ngừng hoạt động bình thường. Đặt địa chỉ IP tĩnh là a chức năng network nâng cao and cần có kiến thức cơ bản về TCP/IP. 
01Initial Setup 
