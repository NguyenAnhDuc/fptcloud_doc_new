...
**Step 1.** Sự kiện kích hoạt luồng: developer có thể kích hoạt luồng triển khai CI/CD với sự kiện **Push commit/Merge request build**
**Step 2.** Hệ thống mã nguồn (source control management) sẽ gửi thông điệp trigger tới hệ thống CI/CD. Hệ thống mã nguồn sẽ gửi thông điệp build qua webhook để kích hoạt một jobs build trên Jenkins
**Step 3.** Hệ thống CI/CD sử dụng FPT Jenkins CI và sử dụng FPT Cloud Agent sẽ tiến hành thực hiện các bước bao gồm: Scan code với sonarqube, Unittest, Build images.
**Step 4.** Nếu Pass qua các bài kiểm thử và build images thành công, images sẽ được đẩy tới hệ thống FPT Container Registry (FCR) để lưu trữ images phục vụ việc deploy ứng dụng.
**Step 5.** Sau khi images được đẩy lên FCR, hệ thống sẽ thực hiện cập nhật tag images mới tới source code chứa config được quản lý bởi FPT Argo CD
**Step 6.** Hệ thống FPT Argo CD thực hiện deploy ứng dụng lên môi trường dev/test/staging
**Step 7.** Sau khi deploy ứng dụng, hệ thống CI/CD thực hiện chạy Automations test để kiểm thử tự động trên môi trường staging
**Step 8.** Sau khi chạy xong luồng CI/CD pipeline, hệ thống CI/CD trả kết quả build về cho developer.
