---
id: "fortigate-fpt-ngfw"
title: "Fortigate Fpt Ngfw"
sidebar_label: "Fortigate Fpt Ngfw"
sidebar_position: "1"
---

# Fortigate – FPT NGFW

  * [ ![](/img/migrated/icon-networking-f57b0b33.png) Hướng dẫn details (Tutorials) ](javascript: "Hướng dẫn details \(Tutorials\)")
  * [ ![](/img/migrated/question-45cd06af.png) Các câu hỏi thường gặp (FAQs) ](javascript: "Các câu hỏi thường gặp \(FAQs\)")
  * [ ![](/img/migrated/suggestion-74a9dc8f.png) Một số gợi ý mẫu (Samples) ](javascript: "Một số gợi ý mẫu \(Samples\)")

Fortigate – FPT NGFW
  1. [FPT Cloud Portal](https://fptcloud.com/danh-sach-tai-lieu/ "FPT Cloud Portal")
  2. Fortigate – FPT NGFW

Fortigate – FPT NGFW
FortiGate NGFW, a phần of Fortinet Security Fabric, là a giải pháp tường lửa with feature tự động bảo vệ khỏi mối đe dọa. Nó is thiết kế to bảo vệ the tổ chức khỏi cả the mối đe dọa bên in and bên ngoài, chẳng hạn như the cuộc tấn công network, phần mềm độc hại (malware) and the lỗ hổng khác. NGFW cung cấp kiểm tra SSL, kiểm soát ứng dụng, khả năng displayed nâng cao and the khả năng security khác.
##  Home page 
01Hướng dẫn details (Tutorials) 
02Các câu hỏi thường gặp (FAQs) 
03Một số gợi ý mẫu (Samples)
