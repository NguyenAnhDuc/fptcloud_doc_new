...
Users can enable the PITR feature for PostgreSQL when activating the Backup service (see [Enabling the backup service](../managed-fpt-database-engines-new/index.md) for details), or enable it after the Backup service has been activated by following the instructions below.
