...
**Step 1** : Ở menu chọn **Security** > **Cloud Guard** , mở tab **Threat** **Alerts** và chọn **Create Alert**.
...
Step 2: Cấu hình cảnh báo theo nhu cầu bằng cách nhập thông tin vào trường dữ liệu tương ứng:  
|   |   |  
| --- | --- |  
| **Field**  | **Description**  |  
...
| **Threat Type**  | Chọn **Loại tấn công** người dùng muốn đặt cảnh báo, gồm có:  |  
  ...
  4. Potential DDoS Attack Detection: IP người dùng có nguy cơ bị tấn công DDoS khi phát hiện có số lượng lớn host đang gửi gói tin tới 1 IP đích trong 1 phút. | | **Apply to** | Chọn các IP mà người dùng muốn bật cảnh báo **Note:** Các IP được phép chọn để bật cảnh báo nằm trong danh sách IP của màn Floating IP | | **Severity** | Chọn mức độ nghiêm trọng của cảnh báo, có các mức: Critical, High, Medium, Low | | **Notify to** | Chọn người nhận cảnh báo |

Step 3: Chọn Create để thực hiện khởi tạo Alert. Hệ thống sẽ thông báo xác nhận, kiểm tra tài nguyên và tiến hành quá trình khởi tạo dựa vào các thông tin đã chọn.
